    ...
    <React.Fragment>
      ...
      {itemsList.length === 0 && itemsState === 'idle' ? (
        <div className="alert alert-light" role="alert">
          По Вашему запросу{q ? ` "${q}" ` : ''} ничего не найдено.
        </div>
      ) : null}

      {!lastFetched && itemsState === 'loading' ? <Preloader /> : null}
      <LoadButton
        firstLoad={loadMore ? nextLoad : firstLoad}
...
const mapStateToProps = (state) => {
  ...
  return { itemsList, itemsState, lastFetched };
};
